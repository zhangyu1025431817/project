...
class DownLoadImageService {
     ...
     * 执行多线程列队执行
     */
    private void runOnQueueCache(Runnable runnable) {
     ...
     * 执行单线程列队执行
     */
    private void runOnQueueSingle(Runnable runnable) {
    ...
    DownLoadImageService(Map<Integer, String> mapUrl, Context context, OnDrawListener listener, boolean isHigh) {
        mListener = listener;
        this.isHigh = isHigh;
        mContext = context;
     ...
     * @param mapUrl
     */
     void drawAll(Map<Integer, String> mapUrl) {
        mIndex = 0;
     ...
     * @param url
     */
    void drawOne(int number, String url, boolean isCancel) {
        if (isCancel) {
    ...
    private class DownLoadThread implements Runnable {
        ...
        DownLoadThread(int index, String url) {
            this.index = index;
            this.url = url;
        }
    ...
    Map<Integer, Bitmap> getBitmapMap(){
        return bitmapMap;
    }
}
