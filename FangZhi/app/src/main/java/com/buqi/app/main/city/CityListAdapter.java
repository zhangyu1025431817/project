...
public class CityListAdapter extends BaseAdapter {
    ...
    public CityListAdapter(Context mContext, List<City> mCities) {
        ...
        mCities.add(0, new City("", "0", "定位", "0"));
        int size = mCities.size();
        letterIndexes = new HashMap<>();
        sections = new String[size];
        for (int index = 0; index < size; index++) {
            //当前城市拼音首字母
            String currentLetter = PinyinUtils.getFirstLetter(mCities.get(index).getArea_ename());
            //上个首字母，如果不存在设为""
            String previousLetter = index >= 1 ? PinyinUtils.getFirstLetter(mCities.get(index - 1).getArea_ename()) : "";
    ...
    public View getView(final int position, View view, ViewGroup parent) {
        ...
        switch (viewType) {
            ...
            case 1:     //所有
                ...
                if (position >= 1) {
                    final String city = mCities.get(position).getArea_cname();
                    holder.name.setText(city);
                    String currentLetter = PinyinUtils.getFirstLetter(mCities.get(position).getArea_ename());
                    String previousLetter = position >= 1 ? PinyinUtils.getFirstLetter(mCities.get(position - 1).getArea_ename()) : "";
                    if (!TextUtils.equals(currentLetter, previousLetter)) {
